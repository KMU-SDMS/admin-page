...
export function PointsManagement({ students, rooms, isLoading }: PointsManagementProps) {
  ...
  const handleSubmitPoint = async (e: React.FormEvent) => {
    ...
    if (!selectedStudentId || !pointForm.type || !pointForm.score || !pointForm.reason.trim()) {
      toast({
        title: "입력 오류",
    ...
    try {
      ...
      toast({
        title: "포인트 부여 실패",
        description: error instanceof Error ? error.message : "포인트 부여에 실패했습니다.",
        variant: "destructive",
      })
    } finally {
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <Card>
        <CardHeader>
          ...
          <SearchInput placeholder="학생명 검색..." value={studentSearch} onChange={setStudentSearch} />
        </CardHeader>
        <CardContent>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredStudents.length === 0 ? (
              <EmptyState title="학생이 없습니다" description="검색 조건에 맞는 학생이 없습니다." />
            ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-sm text-muted-foreground">
                      {student.studentNo} • {room?.name || `호실 ${student.roomId}`}
                    </div>
                  </div>
                )
              })
            ...
            <Card>
              ...
              <CardContent>
                {pointsLoading ? (
                  ...
                  <EmptyState title="포인트 이력이 없습니다" description="아직 부여된 상점이나 벌점이 없습니다." />
                ) : (
                      ...
                      <TableBody>
                        {points.map((point) => (
                          <TableRow key={point.id}>
                            <TableCell>
                              <Badge
                                variant={point.type === "MERIT" ? "default" : "destructive"}
                            ...
                            <TableCell className="font-medium">{point.score}점</TableCell>
                            <TableCell>{point.reason}</TableCell>
                            <TableCell>{new Date(point.createdAt).toLocaleString()}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setPointForm((prev) => ({ ...prev, score: e.target.value }))}
                      />
                    </div>
                  </div>
