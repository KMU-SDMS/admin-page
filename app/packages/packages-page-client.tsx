...
export function PackagesPageClient({
  ...
  const displayParcels = parcels.length > 0 ? parcels : initialParcels;
  ...
  const enrichedParcels: ParcelWithDetails[] = displayParcels.map((parcel) => {
    const student = displayStudents.find((s) => s.id === parcel.studentId);
    const room = displayRooms.find((r) => r.id === parcel.roomId);
    return {
      ...parcel,
  ...
  const carriers = Array.from(
    new Set(displayParcels.map((p) => p.courier))
  ).sort();
